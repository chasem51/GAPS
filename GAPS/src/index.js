  ...
  return (
    <>
      <Block factor={1} offset={0}>
        <Block factor={1.2}>
          <Text left size={w * 0.22} position={[-w / 2, 0.45, -1]} color="#07d4b2">
            GAPS
          </Text>
        </Block>
        <Block factor={1.0}>
          <Dom position={[-w / 2.25, -w * 0.28 + 0.25, -1]}>The intersection of expertise &amp; remedies in fitness.{mobile ? <br /> : " "}&copy;2020</Dom>
        </Block>
      </Block>
      <Block factor={1.2} offset={5.7}>
        <MultilineText top left size={w / 28} lineHeight={w / 10} position={[-w / 2, 0, -1]} color="#2fe8c3" text={"Project Lead: Levi Golden\nTechnical Lead: Chase Maivald\nExcercise Scientist: Gordon Levine\nSpecification Lead: Ben Smith"} />
      </Block>
  ...
  return (
    <>
      <Canvas className="canvas" concurrent pixelRatio={1} orthographic camera={{ zoom: state.zoom, position: [0, 0, 500] }}>
        <Suspense fallback={<Dom center className="loading" />}>
          <Content />
          <Diamonds />
          <Startup />
        </Suspense>
      ...
      <div className="frame">
        <h1 className="frame__title">A health + wellness project.</h1>
